...
describe('Testing endpoints for flights resource', () => {
  it('POST "/flights/checkins" should return for correct request created boarding pass with seat number and correct status', async () => {
    ...
    return request.post('/bookings').send(booking).then((response) => {
      ...
      return request.post('/flights/checkins').send({ ticketNo, flightId: 11057 }).then((res) => {
  ...
  it('POST "/flights/checkins" should return BadRequest error with message when trying to re-check in for a flight', async () => {
    ...
    return request.post('/bookings').send(booking).then((response) => {
      expect(response.status).toBe(201);
      const { ticketNo } = response.body;
      const boardingPass = { ticketNo, flightId: 11063 };
      return request.post('/flights/checkins').send(boardingPass).then((res1) => {
        expect(res1.status).toBe(201);
        return request.post('/flights/checkins').send(boardingPass).then((res2) => {
  ...
  it('POST "/flights/checkins" should return for non-existing flight NotFound error with message', async () => {
    const booking = {
      ...
      flightDate: '2025-12-01',
      fareConditions: 'Economy',
      passengerId: '_checkin_03',
      passengerName: 'NotFound Test',
    };
    return request.post('/bookings').send(booking).then((response) => {
      expect(response.status).toBe(201);
      const { ticketNo } = response.body;
      return request.post('/flights/checkins').send({ ticketNo, flightId: 9999999 }).then((res) => {
  ...
  it('POST "/flights/checkins" should return for incorrect request body BadRequest error with message', async () => {
    return request.post('/flights/checkins').send({ flightId: null }).then((response) => {
